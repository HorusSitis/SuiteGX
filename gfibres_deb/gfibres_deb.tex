...
\subsubsection{Une construction de $\mathbb{U}\hookrightarrow\mathbb{S}^2\twoheadrightarrow\mathbb{S}^2$ \g{a} l'aide des quaternions}
...
\begin{description}
\item[Injectivit\'e de $\mathcal{P}_{|\exp(U)}$ :] % Montrons que $\mathcal{P}$ est injective sur $\exp(U)$.
%
%\par
...
\par
Il reste \g{a} montrer que que $\mathcal{P}(U)$ est un voisinage de l'unit\'e dans $G/H$.
...
\par
R\'eciproquement, soit $S$ un quaternion de carr\'e \'egal \g{a} $-1$. On peut \'ecrire : $S^{-1}=-S$, donc $\dfrac{S^{\ast}}{\|S\|^2}=-S$. %
...
Remarquons que :
\begin{itemize}
\item $\mathbb{R}\mathbf{I}$ est l'espace tangent \g{a} $SU_2(\mathbb{C})_{\mathbf{I}}$ en $I_2$ ;
...
\par
De m\^eme que dans la d\'emonstration g\'en\'erale, on utilise la restriction de l'exponentielle \g{a} l'espace $\mathbb{R}\mathbf{J}\oplus\mathbb{R}\mathbf{K}$.

\par
Ce dernier espace, inclus dans $\mathcal{V}$ d'apr\g{e}s la remarque pr\'ec\'edente, est constitu\'e des matrices de la forme :
...
Cette derni\g{e}re matrice, qui est un quaternion, est le p\^ole Nord ou le point de coordonn\'ees sph\'eriques
\[(1,|b|,\arg b)\]
...
sur la sph\g{e}re-unit\'e, incluse dans $SU_2(\mathbb{C})$, de l'espace r\'eel $Vect_{\mathbb{R}}(I_2,\mathbf{J},\mathbf{K})$ inclus dans $\mathbb{H}$ mais pas dans $\mathcal{V}$, %
que l'on prive de son p\^ole Sud.

%\par
%On prend ce disque $\mathcal{D_{\pi}}$ pour l'ouvert $U$ de la d\'emonstration du th\'eor\g{e}me \ref{esh}.

\par
On recherche paintenant une section de la projection $SU_2(\mathbb{C})\rightarrow \mathbb{S}^2 : U\mapsto \chi (U,\mathbf{I})$ %
o\g{u} $\mathbb{S}^2$ est entendue comme \'etant la sph\g{e}re-unit\'e de l'espace des quaternions purs.

\par
Soit $\mcd{0}{-b}{\overline{b}}{0}$ une matrice %\emph{non nulle} %
...
On distingue maintenant le cas o\g{u} $b=0$ : \[\chi \left(\exp\mcd{0}{-0}{\overline{0}}{0},\mathbf{I}\right)=\mathbf{I}\]
de celui o\g{u} $b$ est non nul ; on peut alors \'ecrire :
...
donc, d'apr\g{e}s les formules de trigonom\'etrie :
...
\par
Celui-ci peut s'\'ecrire de deux mani\g{e}res, en choisissant les bases $(I_2,\mathbf{J},\mathbf{K}) $ et $(\mathbf{I},\mathbf{J},\mathbf{K}$ %
...
o\g{u} $B(\cdot,\cdot)$ et $S(\cdot,\cdot)$ sont les points de $\exp(\mathcal{D}_{\frac{\pi}{2}})$ et $\mathbb{S}_{\mathcal{V}}^2$ rep\'er\'es par leurs coordonn\'ees sph\'eriques.
%
\item[Restriction \g{a} $\exp(\mathcal{D}_{\frac{\pi}{2}}$ d'une fonction polyn\^omiale :] D'apr\g{e}s l'\'equation \ref{dbcl} %
...
une section de la projection $B\mapsto\chi(B,\mathbf{I})$ de $SU_2(\mathbb{C})$ sur son espace homog\g{e}ne $\mathbb{S}^2$, %
d\'efinie sur cette sph\g{e}re priv\'ee de son p\^ole Sud.
